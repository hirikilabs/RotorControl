...
func (p* Rot2ProgPacket) GenBytes() error {
	...
	case Set:
		...
		azData := float64(p.DegPulse) * (360.0 + p.Az)
		elData := float64(p.DegPulse) * (360.0 + p.El)
		// convert to XXX.X zero padded
		azString := fmt.Sprintf("%03.1f", azData)
		elString := fmt.Sprintf("%03.1f", elData)
		// fill bytes as numbers
		p.Bytes[1] = azString[0] - 48
		p.Bytes[2] = azString[1] - 48
		p.Bytes[3] = azString[2] - 48
		p.Bytes[4] = azString[4] - 48

		p.Bytes[5] = DegPulse05

		p.Bytes[6] = elString[0] - 48
		p.Bytes[7] = elString[1] - 48
		p.Bytes[8] = elString[2] - 48
		p.Bytes[9] = elString[4] - 48
